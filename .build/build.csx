...
dotNetBuild.AddTarget("updateVersionNumber", "Update version number", c 
    => c.Do(context => {
            ...
            var assemblyInfoTask = new AssemblyInfo
            {
                ...
                {
                    Path.Combine(solutionDirectory, @"Glimpse.SignalR\Properties\AssemblyInfo.cs")
                },
...
dotNetBuild.AddTarget("buildRelease", "Build in release mode", c 
	=> c.Do(context => {
			...
			{
				Project = Path.Combine(solutionDirectory, "Glimpse.SignalR.sln"),
				Target = "Rebuild",
...
dotNetBuild.AddTarget("createPackage", "Create NuGet package", c 
    => c.Do(context => {
            ...
            var nugetPackTask = new Pack
            {
                ...
                NuSpecFile = Path.Combine(solutionDirectory, @"Glimpse.SignalR\package.nuspec"),
...
dotNetBuild.AddTarget("publishPackage", "Publish NuGet package", c
	=> c.Do(context => {
            ...
            var nupkgFile = string.Format(@"packagesForNuGet\Glimpse.SignalR.{0}.nupkg", context.ParameterProvider.Get("VersionNumber"));
...
dotNetBuild.AddConfiguration("defaultConfig", c 
	=> c.AddSetting("SolutionDirectory", @"..\")
        .AddSetting("PathToNuGetExe", @".nuget\NuGet.exe")
